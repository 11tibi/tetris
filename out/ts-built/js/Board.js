...
var GamePhase;
(function (GamePhase) {
    GamePhase[GamePhase["PLAY"] = 0] = "PLAY";
    GamePhase[GamePhase["GAME_OVER"] = 1] = "GAME_OVER";
})(GamePhase || (GamePhase = {}));
export class Board {
    constructor(ctx) {
        this.ctx = ctx;
    ...
    draw() {
        ...
        this.ctx.rect((window.innerWidth / 2) - (BOARD.SQUARE_WIDTH * BOARD.SQUARES_X) / 2 - 3, (window.innerHeight / 2) - (BOARD.SQUARE_HEIGHT * BOARD.SQUARES_Y) / 2 - 3, BOARD.SQUARE_WIDTH * BOARD.SQUARES_X + 5, BOARD.SQUARE_HEIGHT * BOARD.SQUARES_Y + 5);
        this.ctx.fill();
        ...
        for (let i = 0; i < BOARD.SQUARES_Y; i++) {
            for (let j = 0; j < BOARD.SQUARES_X; j++) {
                if (this.fields[i][j] != -1) {
                    ...
                    this.ctx.fillRect(BOARD.BOARD_START_X + j * BOARD.SQUARE_WIDTH, BOARD.BOARD_START_Y + i * BOARD.SQUARE_HEIGHT, BOARD.SQUARE_WIDTH, BOARD.SQUARE_HEIGHT);
                }
            }
        }
    }
    ...
    checkCollision() {
        const currFigure = BOARD.FIGURES[this.currentPiece.getType()][this.currentPiece.getRotation() % (BOARD.FIGURES[this.currentPiece.getType()].length)];
        for (let i = 0; i < 4; i++) {
            for (let j = 0; j < 4; j++) {
                if (currFigure.includes(i * 4 + j)) {
                    let x_ind = this.currentPiece.getX() + j;
                    let y_ind = this.currentPiece.getY() + i;
